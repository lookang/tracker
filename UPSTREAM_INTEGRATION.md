# SwingJS integration guide: Tracker Student Mobile

This document records how the independent `StudentMobile` branch follows
Tracker's active SwingJS development line. It separates canonical source
changes from launcher code, generated artifacts, and site-specific content so
each layer can be evaluated and updated independently.

## Branch relationship

- Upstream source: `OpenSourcePhysics/tracker:SwingJS`
- Matching core dependency: `OpenSourcePhysics/osp:swingJS`
- Independent mobile branch: `lookang/tracker:StudentMobile`
- Initial SwingJS base: `70be500e` (29 July 2026)
- Imported mobile implementation: `34890663`, reapplied as a content change
  rather than by merging its older `master` ancestry

`OpenSourcePhysics/tracker:SwingJS` is the upstream default and contains the
Tracker 6.2.0-and-newer development history. `StudentMobile` is a separate
product branch. The former master-targeting proposal was retired after
maintainer guidance; no upstream merge is implied by this branch.
...
## Fetch the StudentMobile branch

From an existing Tracker checkout:

```bash
git remote add lookang https://github.com/lookang/tracker.git
git fetch lookang StudentMobile
git switch -c review/tracker-student-mobile \
  --track lookang/StudentMobile
```

If a `lookang` remote already exists, omit the first command.
...
`cdn_cores/tracker6.1.6/core_tracker.z.js` is included because that legacy-named
path contains the exact core deployed with the verified public build. The
directory name does not describe the current `StudentMobile` Java source,
which starts from the Tracker 6.3.5-era SwingJS branch. The core contains
generated forms of the Java changes plus compatibility fixes whose canonical
source belongs to a different OSP/SwingJS dependency.
...
## Safe upstream synchronization sequence

1. Fetch the latest `OpenSourcePhysics/tracker:SwingJS` and the matching
   `OpenSourcePhysics/osp:swingJS` dependency.
2. Create a temporary update branch from the current `StudentMobile`.
3. Merge `upstream/SwingJS` into that update branch; never merge the obsolete
   master-based mobile branch.
4. Review the three Java source changes independently against the new source.
5. Regenerate the SwingJS core from canonical sources.
6. Compare and, where still necessary, apply the signature-checked core patches.
7. Refresh the isolated `swingjs2-tracker-mobile.js` from the matching upstream
   runtime and reapply its documented touch and long-arithmetic corrections.
8. Run the complete mobile checklist against HTTP(S), not `file:///`.
9. Test current iPhone Safari and Android Chrome in both orientations, followed
   by tablet and desktop regression checks.
10. Test local files and ComPADRE, Tracker Home, and Shared Library projects.
11. Record the resulting core/launcher hashes and use a new cache key.
12. Merge the tested update branch into `StudentMobile` and push it to the fork.
