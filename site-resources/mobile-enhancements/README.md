...
Active source branch: `lookang/tracker:StudentMobile`, based directly on
...
<https://iwant2study.org/tracker/TrackerStudentMobilePedagogy.html?v=20260809-sm32#mobile-mission>

Teacher video tutorial:
<https://youtu.be/6G6A_feou5Q>
...
this launcher. Its intentional differences provide touch/pointer tap delivery,
logical coordinate remapping for reflowed Tracker controls, duplicate
compatibility-click suppression, and the three `$`-prefixed long operators in
`Long.$mod`. Keeping it launcher-local avoids changing the standard Tracker
pages. Whenever `upstream/SwingJS` updates its runtime, regenerate this copy
from the matching `swingjs2.js`, review the complete diff, and retest every
touch path.
...
The source launcher uses SwingJS's standard `core: "_tracker"` selector so it
loads the core produced by the current SwingJS build. Deployment cache keys
version the launcher and mobile assets; publishers must also invalidate or
version the generated core when it changes.
