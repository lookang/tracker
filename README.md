...
`StudentMobile` is the active development branch for this variant. It was
created directly from `OpenSourcePhysics/tracker:SwingJS`, the upstream default
branch and Tracker 6.3.x development line, on 29 July 2026. The earlier
master-based prototype remains available only as migration history.

The Student Mobile project is intentionally maintained separately from the
standard Tracker application. Updates from upstream should be integrated into
`StudentMobile`, tested on phone-sized layouts, and then pushed to
`lookang/tracker:StudentMobile`. Mobile changes should not be proposed back to
OpenSourcePhysics unless the Tracker maintainers request a focused contribution.

## Try it

| Resource | Link |
| --- | --- |
| Tracker Student Mobile, current verified build | [Open build 20260730-sm14](https://iwant2study.org/tracker/TrackerStudentMobile.html?v=20260730-sm14) |
...
- `swingjs2-tracker-mobile.js`: launcher-local SwingJS touch/pointer mapping and
  long-arithmetic compatibility runtime.
...
See [UPSTREAM_INTEGRATION.md](UPSTREAM_INTEGRATION.md) for the branch lineage,
change-by-change review, and safe SwingJS synchronization procedure.
...
| `cdn_cores/tracker6.1.6/core_tracker.z.js` | Generated core used by the public mobile build. The directory name is a retained deployment path, not the current Java source version. |
...
- Apache Ant;
- the matching `swingJS` branch of the OSP Core Library from
...
## SwingJS synchronization

This branch follows `OpenSourcePhysics/tracker:SwingJS` without merging the
older master-based prototype history. To bring in a new upstream release,
fetch `upstream/SwingJS` and merge it into a temporary update branch created
from `StudentMobile`. Rebuild the generated core, reapply only compatibility
patches that are still necessary, run the full phone-first checklist, and then
merge the tested update into `StudentMobile`.

Do not replace this history with the old `master` branch and do not assume that
a conflict-free generated-file merge is a valid rebuild. Start with
[UPSTREAM_INTEGRATION.md](UPSTREAM_INTEGRATION.md), then review the Java source,
launcher layer, isolated SwingJS runtime, deterministic core patches, and
verification artifacts separately.
